...
function find_item(table_id, filiale_id, item_name, type,tab){
    let url = "";
    if (type === "ARTICLE") url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/subsidiaries/" + filiale_id + "/itemsInfo/" + item_name;
    else url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/externalEntities/" + (type === "CLIENT" ? "0" : "1") + "/" + filiale_id;
    ...
    $(document).on("keyup",input_id, function (){
        ...
        if (item_name !== ''){
            ...
            if (finded_item === undefined){
                ...
                let url = "";
                if (item_type === "ARTICLE") url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/subsidiaries/" + filiale_id + "/itemsInfo/" + item_name;
                else url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/externalEntities/" + (type === "CLIENT" ? "0" : "1") + "/" + filiale_id;
                execute_ajax_request("get", url, null, (data) => append_data_to_table(table_id, item_type, data, tab))
...
function fetch_trosa_data(namespace,$cf_id){
    let url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/trosas/cf/" + $cf_id;
...
const init_payement_dette_btn = (namespace,filiale_id,user_id)=>{
    ...
    function update_reste(namespace,montant_payer,type_trosa) {
        ...
        execute_ajax_request("put", "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/trosas/" + $trosa_id,montant_reste, (data) => {
...
const fetch_all_cf_and_append_to_table = (type,namespace,table,$filiale_id)=>{
    let url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/externalEntities/"+type+"/"+$filiale_id;
    execute_ajax_request("get",url,null,(data)=> {
...
const  init_seach_cf_btn = (type,namespace,table,$filiale_id)=>{
    ...
    $(namespace+"#search-btn").click(()=>{
        ...
        if (undefined !== text && text!==""){
            let url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/externalEntities/"+type+"/"+$filiale_id+"/name/"+text;
