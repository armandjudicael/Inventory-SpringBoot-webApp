$(function () {
    $fonctionUrl = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/fonctions";
    $userUrl = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/users";
    $NEW_USER = true;
    ...
    */
    ...
    const init_form = ($userId)=>{
        let url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/users/"+$userId;
    ...
    $(document).on("click",namespace+"#search-btn",()=>{
        ...
        if (text!==""){
            let url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/users/name/"+filiale_id+"/"+text;
            execute_ajax_request("get",url,null,(data)=> {
    ...
    $(document).on("click",namespace+"#refresh-btn",()=>{
        let url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/subsidiaries/"+filiale_id+"/users";
        execute_ajax_request("get",url,null,(data)=> {
            clear_table(namespace+"#table-liste-utilisateur");
            append_user_item(data)
        });
