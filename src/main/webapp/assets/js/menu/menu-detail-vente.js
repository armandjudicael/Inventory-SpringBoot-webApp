 $(function(){
    ...
    $(document).on('change',namespace+"#magasin-select-operation",function(){
        ...
        let url = _url + "api/v1/stores/"+magasinId+"/sales"
        getAllSales(url);
    });
    // NAME FILTER
    $(namespace+"#btn-search-operation").click(()=>{
        ...
        if (text!==undefined && text!=="") url = _url + "api/v1/sales/subsdiary/"+filiale_id+"/filter-type/"+typeFilter+"/"+text.toLowerCase();
    ...
    $(namespace+"#search-button").click(()=>{
        ...
        let filiale_id = $(namespace + '#filiale-id').attr("value-id");
        let url = _url + "api/v1/sales/subsdiary/"+filiale_id+"/between-date/"+beginDate+"/"+endDate;
        getAllSales(url);
    });
     ...
     */
     ...
     //fSearch(namespace + '#input-search-operation', namespace + '#details-vente-table tbody tr');
