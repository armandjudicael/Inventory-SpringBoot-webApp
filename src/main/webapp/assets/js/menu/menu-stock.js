$(function () {
    ...
    const TOUTES = "";
    const INVENTORY_URL = _url + "api/v1/inventories/"+filiale_id;
    const STORE_INVENTORY_URL = _url + "api/v1/inventories/stores/";
    ...
    $(namespace+"#btn-article-alert").click(()=>{
        ...
        let url = _url + "api/v1/subsidiaries/"+$filialeId+"/inventories";
    ...
    function fetchInventoryActivities(magasinId, articleId, uniteId){
        let url = _url + "api/v1/activities/" + magasinId + "/" + articleId + "/" + uniteId;
    ...
    function fetchInventoryAlert(articleId,filialeId){
        let url = _url + "api/v1/inventories-alert/" + filialeId + "/" + articleId;
        execute_ajax_request("get", url, null, (data) => {
    ...
    $(document).on("keyup",namespace+"#stock-search",()=>{
        ...
        if (name!==""){
            let url = _url + "api/v1/subsidiaries/"+filialeId+"/inventories/"+name;
            execute_ajax_request("get",url,null,(data)=>{
                clear_table(namespace+"#inventory-table")
                $.each(data,(key,value)=>{
                    let tr = `
    ...
    $(document).on('change',namespace+"#magasin-select",function(){
        let val = $(this).val();
        let url = val===TOUTES ? INVENTORY_URL : _url + "api/v1/magasins/"+val+"/inventories";
        execute_ajax_request("get",url,null,(data)=>append_inventory_item_to_table(data));
        update_montant_lable( val==TOUTES ? _url + "api/v1/inventories/"+filiale_id+"/total-values" : _url + "api/v1/inventories/"+filiale_id+"/store/"+val+"/total-values");
    ...
    $('.btn-stock-valeur').click(function(){
        $('.s-value').toggle();
        $('.s-no-value').toggle();
        let  url = _url + "api/v1/inventories/"+filiale_id+"/total-values";
    ...
    const update_montant_lable = (url) => {
        execute_ajax_request("get",url,null,(data)=> {
            ...
            $('.label-marge-stock').text(" ; Marge en stock : " + (data*0.75) + " Ar");
        })
    }
