$(function () {
    ...
    const TOUTES = "";
    const INVENTORY_URL = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/inventories/"+filiale_id;
    const STORE_INVENTORY_URL = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/inventories/stores/";
    ...
    $(namespace+"#btn-article-alert").click(()=>{
        ...
        let url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/subsidiaries/"+$filialeId+"/inventories";
    ...
    function fetchInventoryActivities(magasinId, articleId, uniteId){
        let url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/activities/" + magasinId + "/" + articleId + "/" + uniteId;
    ...
    function fetchInventoryAlert(articleId,filialeId){
        let url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/inventories-alert/" + filialeId + "/" + articleId;
        execute_ajax_request("get", url, null, (data) => {
    ...
    $(document).on("keyup",namespace+"#stock-search",()=>{
        ...
        if (name!==""){
            let url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/subsidiaries/"+filialeId+"/inventories/"+name;
            execute_ajax_request("get",url,null,(data)=>{
                clear_table(namespace+"#inventory-table")
                $.each(data,(key,value)=>{
                    let tr = `
    ...
    $(document).on('change',namespace+"#magasin-select",function(){
        let val = $(this).val();
        let url = val===TOUTES ? INVENTORY_URL : "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/magasins/"+val+"/inventories";
        execute_ajax_request("get",url,null,(data)=>append_inventory_item_to_table(data));
        update_montant_lable( val==TOUTES ? "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/inventories/"+filiale_id+"/total-values" : "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/inventories/"+filiale_id+"/store/"+val+"/total-values");
    ...
    $('.btn-stock-valeur').click(function(){
        $('.s-value').toggle();
        $('.s-no-value').toggle();
        let  url = "http://80.241.220.194:8080/admin-0.0.1-SNAPSHOT/api/v1/inventories/"+filiale_id+"/total-values";
