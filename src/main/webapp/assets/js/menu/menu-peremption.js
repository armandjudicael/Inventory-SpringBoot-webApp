$(function () {
    ...
    $(document).on('dblclick','.table-peremption tbody tr',function () {
        ...
        $("#modal-date-peremption #btn-enregistrer-date-peremption").on('click', function(){
            ...
            let url = "http://80.241.220.194:8080/api/v1/expirations/"+magasin_id+"/"+article_id+"/"+unite_id;
    ...
    * */
    ...
    $(document).on("click",namespace+"#search-btn",()=>{
        ...
        if (product_name!==""){
            ...
            let url = "http://80.241.220.194:8080/api/v1/expirations/"+filialeId+"/"+product_name;
    ...
    * */
    ...
    $(namespace+".btn-status").click(function (){
        ...
        let filialeId = $(namespace + '#filiale-id').attr("value-id");
        let url ="http://80.241.220.194:8080/api/v1/expirations/"+filialeId+"/status/"+expiration_status;
        execute_ajax_request("get",url,null,(expiration_data)=> appendExpirationData(expiration_data));
    })
    /*
    ...
    * */
    ...
    $(document).on('change',namespace+"#magasin-select-item",function(){
        ...
        let url = "";
        let filialeId = $(namespace + '#filiale-id').attr("value-id");
        if (magasinId!=="toutes") url = "http://80.241.220.194:8080/api/v1/expirations/"+magasinId;
        else url ="http://80.241.220.194:8080/api/v1/expirations/"+filialeId+"/status/toutes";
